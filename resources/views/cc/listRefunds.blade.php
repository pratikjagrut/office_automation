...
@section('content')
									...
									@foreach ($refunds as $refund)
										<tr>
											...
											<td>
												...
												@if ($refund->mode_of_payment == 'cash' && $refund->refund_status == 'granted')
													<a class="btn btn-warning btn-sm pull-right" style="color: white;" data-toggle="modal" data-target="#cashDone" id="{{$refund->id}}/{{$refund->mode_of_payment}}/{{$refund->customer_id}}" onclick="cashDone(this.id)">
														<img src="/storage/images/pencil.png" height="13px">
													</a>
												@else
													@if ($refund->mode_of_payment != 'cash' && ($refund->refund_status == 'granted' || $refund->refund_status == 'done'))
	...
	<!-- Modal -->
	      ...
	      <div class="modal-content">
	        ...
	        <div class="modal-body">
				<form action="/updateUtr" method="post">
					...
					<table class="table table-striped">
						<tr class="form-group">
							<td><label>Enter UTR No/Cheque No:</label></td>
							<td>
	  ...
	  <!-- Modal -->
	  <div class="modal fade" id="cashDone" role="dialog">
	    <div class="modal-dialog">
	      ...
	      <div class="modal-content">
	        <div class="modal-header">
	          <button type="button" class="close" data-dismiss="modal">&times;</button>
	          <h4 class="modal-title text-center"><b>Complete cash refund</b></h4>
	        </div>
	        <div class="modal-body">
				<form action="/updateUtr" method="post">
					{{csrf_field()}}
					<table class="table table-striped">
						<tr class="form-group">
							<td><label>Customer Id:</label></td>
							<td>
								<input type="text" name="customer_id" class="form-control" required="true" readonly="true" id="customer_id3">
							</td>
						</tr>
						<tr class="form-group">
							<td><label>Mode of payment:</label></td>
							<td>
								<input type="text" name="modeOfPayment" class="form-control" readonly="true" required="true" id="modeOfPayment">
							</td>
						</tr>	
						<tr class="form-group">
							<td>
								<input type="hidden" name="refund_id" id="refund_id2">
							</td>
							<td>
								<input type="submit" class="btn btn-success" name="cashDone" value="Done">
							</td>
						</tr>
					</table>
				</form>
	...
	<script type="text/javascript">
		function cashDone(id)
		{	
			var data = id.split('/')
			console.log(id)
			document.getElementById("refund_id2").value = data[0]
			document.getElementById("modeOfPayment").value = data[1]
			document.getElementById("customer_id3").value = data[2]
